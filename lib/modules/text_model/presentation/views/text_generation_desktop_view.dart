...
class TextGenerationDesktopView extends StatefulWidget {
  const TextGenerationDesktopView({super.key});

  @override
  State<TextGenerationDesktopView> createState() => _TextGenerationDesktopViewState();
}

class _TextGenerationDesktopViewState extends State<TextGenerationDesktopView> {
  @override
  ...
  Widget _buildContent(BuildContext context) {
    return BlocProvider(
      create: (context) => serviceLocator<TextGenerationBloc>(),
                  ...
                  child: SizedBox(
                    ...
                    child: ListView.separated(
                      itemBuilder: (context, index) => Row(
                        mainAxisAlignment: BlocProvider.of<TextGenerationBloc>(context, listen: true).conversations[index].isBot
                            ? MainAxisAlignment.start
                            : MainAxisAlignment.end,
                        children: [
                          MessageTileWidget(
                            profileImage:
                                BlocProvider.of<TextGenerationBloc>(context, listen: true).conversations[index].profile ??
                                    "assets/images/omni-suite_logo.png",
                            prompt: BlocProvider.of<TextGenerationBloc>(context, listen: true).conversations[index].prompt ?? "",
                            isBot: BlocProvider.of<TextGenerationBloc>(context, listen: true).conversations[index].isBot,
                          ),
                        ],
                      ),
                      separatorBuilder: (context, index) => UIHelpers.verticalSpaceSmall,
                      itemCount: BlocProvider.of<TextGenerationBloc>(context, listen: true).conversations.length,
                    ),
                  ),
                ),
                UIHelpers.verticalSpaceRegular,
                  ...
                  builder: (context, state) {
                    return Expanded(
                      child: CustomTextField(
                        controller: BlocProvider.of<TextGenerationBloc>(context).promptController,
                            ...
                            : (p0) {
                                if (p0 != " " && p0.isNotEmpty) {
                                  BlocProvider.of<TextGenerationBloc>(context).add(
                                    GenerateTextEvent(
                                      messageAttributesModel: MessageAttributesModel(
                                        profile: "assets/images/omni-suite_logo.png",
                                        prompt: BlocProvider.of<TextGenerationBloc>(context).promptController.text,
                                        isBot: false,
                                      ),
                                    ),
                                  );
                                  BlocProvider.of<TextGenerationBloc>(context).promptController.clear();
                        ...
                        suffixIcon: InkWell(
                          onTap: state is TextGenerationLoadingState
                              ? null
                              : () {
                                  if (BlocProvider.of<TextGenerationBloc>(context).promptController.text != " " &&
                                      BlocProvider.of<TextGenerationBloc>(context).promptController.text.isNotEmpty) {
                                    BlocProvider.of<TextGenerationBloc>(context).add(
                                      GenerateTextEvent(
                                        messageAttributesModel: MessageAttributesModel(
                                          profile: "assets/images/omni-suite_logo.png",
